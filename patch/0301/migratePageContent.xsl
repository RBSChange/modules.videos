...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template
		match="change:block[@type = 'modules_videos_video' or @type ='modules_videos_playlist']">
		<change:block>
			<xsl:for-each select="@*">
				<xsl:choose>
					<xsl:when test="name(.) = '__width'">
						<xsl:if test=". != ''">
							<xsl:attribute name="__videoWidth"><xsl:value-of
								select="." /></xsl:attribute>
						</xsl:if>
					</xsl:when>
					<xsl:when test="name(.) = '__height'">
						<xsl:if test=". != ''">
							<xsl:attribute name="__videoHeight"><xsl:value-of
								select="." /></xsl:attribute>
						</xsl:if>
					</xsl:when>
