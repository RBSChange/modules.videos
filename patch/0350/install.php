 ...
 */
class videos_patch_0350 extends patch_BasePatch
{
	...
	public function execute()
	{
		...
		$scriptPath = "modules/videos/patch/0350/migratePageContent.php";
		...
		if (f_util_ArrayUtils::isEmpty($this->executeSQLSelect("DESC `m_videos_doc_preferences` `videowidth`")->fetchAll()))
		{
			$this->log("rename width to videowidth");
		...
		if (f_util_ArrayUtils::isEmpty($this->executeSQLSelect("DESC `m_videos_doc_preferences` `videoheight`")->fetchAll()))
		{
			$this->log("rename height to videoheight");
